...
@Getter
@Setter
public class WriteEventSourceListener extends EventSourceListener {
    ...
    public WriteEventSourceListener(WorkUserWrite workUserWrite, SseEmitter sseEmitters) {
        this.workUserWrite = workUserWrite;
        this.sseEmitter = sseEmitters;
    }

    private WriteEventSourceListener() {
...
//        workUserWrite.setWords(workUserWrite.getWords() + (long) workUserWrite.getContent().length());
        workUserWrite.setWords(workUserWrite.getWords()/2 + (long) workUserWrite.getContent().length()/2);
