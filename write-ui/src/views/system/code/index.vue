<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="剩余字数" prop="remainingWords">
        <el-input
          v-model="queryParams.remainingWords"
          placeholder="请输入剩余字数"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="codeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="剩余字数" align="center" prop="remainingWords" />
      <el-table-column label="使用截止时间" align="center" prop="deadline" width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="剩余字数" prop="remainingWords">
          <el-input v-model="form.remainingWords" placeholder="请输入剩余字数" />
        </el-form-item>
        <el-form-item label="使用截止时间" prop="deadline">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        remainingWords: [
          { required: true, message: "剩余字数不能为空", trigger: "blur" }
        ],
        deadline: [
          { required: true, message: "使用截止时间不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户兑换码编号为"' + ids + '"的数据项？').then(function() {
        return delCode(ids);
      }).then(() => {
        this.getList();
